...
export default function AddReservationScreen() {
  const params = useLocalSearchParams<{ listing_id?: string; checkin?: string; checkout?: string }>();
  const [listings, setListings] = useState<Listing[]>([]);
  const [selectedListing, setSelectedListing] = useState<number | null>(params.listing_id ? parseInt(params.listing_id, 10) : null);
  const [listingSearch, setListingSearch] = useState('');
  const [guestName, setGuestName] = useState('');
  const [guestPhoto, setGuestPhoto] = useState<any>(null);
  const [checkin, setCheckin] = useState<Date>(params.checkin ? new Date(params.checkin) : new Date());
  const [checkout, setCheckout] = useState<Date>(params.checkout ? new Date(params.checkout) : new Date(Date.now() + 86400000));
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await fetch(API.reservationCreate, { method:'POST', body: form });
      if (res.ok) {
    ...
    <View style={s.container}>
      ...
      <ScrollView contentContainerStyle={s.body} keyboardShouldPersistTaps="handled">
        ...
        <TouchableOpacity style={[s.submitBtn, loading && s.btnDisabled]} onPress={submit} disabled={loading}>
          {loading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={s.submitText}>Create Booking</Text>}
        </TouchableOpacity>
